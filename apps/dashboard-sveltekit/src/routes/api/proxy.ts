...
async function proxyToN8n(action: string, data: any = {}) {
  // Gunakan environment variable untuk base URL n8n sebenarnya
...
// Endpoint GET untuk endpoint spesifik bisa ditambahkan sesuai kebutuhan
export const GET: RequestHandler = async ({ url }) => {
  try {
    const action = url.searchParams.get('action');
    
    if (!action) {
      return json({ error: 'Action parameter is required for GET requests' }, { status: 400 });
    }

    const result = await proxyToN8n(action);
    return json(result);
  } catch (error) {
