...
// Base URL Logic:
// Strictly use REACT_APP_API_BASE_URL from environment variables.
// No fallback to localhost to avoid hidden configuration behavior.
const baseUrl = process.env.REACT_APP_API_BASE_URL || '';

// Remove trailing slash and add /api if not already present
const cleanUrl = baseUrl.replace(/\/$/, '');
const API_BASE_URL = cleanUrl.endsWith('/api') ? cleanUrl : (cleanUrl ? cleanUrl + '/api' : '');
...
if (!API_BASE_URL) {
  console.warn('⚠️ API_BASE_URL is empty. Please set REACT_APP_API_BASE_URL environment variable.');
}
